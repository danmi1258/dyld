											...
											const linkedit_data_command** codeSigCmd)
{
	...
	*codeSigCmd = NULL;
	...
	const uint32_t cmd_count = mh->ncmds;	//number of cmds
	const struct load_command* const startCmds = (struct load_command*)(((uint8_t*)mh) + sizeof(macho_header));//this is the start address of the command.
	const struct load_command* const endCmds   = (struct load_command*)(((uint8_t*)mh) + sizeof(macho_header) + mh->sizeofcmds);//end
	const struct load_command* cmd = startCmds; //the first command
	
	for (uint32_t i = 0; i < cmd_count; ++i) {
		switch (cmd->cmd) {
			case LC_DYLD_INFO:
			case LC_DYLD_INFO_ONLY:
				*compressed = true;		//如果出现LC_DYLD_INFO(_ONLY)出现了， 那么设置标志位compressed为1
				break;
			case LC_SEGMENT_COMMAND:
				segCmd = (struct macho_segment_command*)cmd;
				// ignore zero-sized segments
				if ( segCmd->vmsize != 0 )	//判断当前的segment的虚拟内存的大小是否为0
					*segCount += 1;
				// <rdar://problem/7942521> all load commands must be in an executable segment
				
				//This is the _TEXT_ segment
				//the judgment, "if ( context.codeSigningEnforced && (segCmd->fileoff < mh->sizeofcmds) && (segCmd->filesize != 0) ) {",
				//is for test _TEXT_ segment but not others, especially _PAZE_ZERO_
				if ( context.codeSigningEnforced && (segCmd->fileoff < mh->sizeofcmds) && (segCmd->filesize != 0) ) {
					...
					if ( segCmd->initprot != (VM_PROT_READ | VM_PROT_EXECUTE) )		//the read and write permission is needed
					...
					loadCommandSegmentIndex = i; //记录下当前_TEXT_是在数值的那个位置
					loadCommandSegmentVMStart = segCmd->vmaddr;	//_TEXT_起始虚拟地址的位置
					loadCommandSegmentVMEnd   = segCmd->vmaddr + segCmd->vmsize; //end of virtual address
				}
				break;
	...
	if ( context.codeSigningEnforced ) {
		for (uint32_t i = 0; i < cmd_count; ++i) {
			switch (cmd->cmd) {
				case LC_SEGMENT_COMMAND:
					if ( i != loadCommandSegmentIndex ) {//if it is not the _TEXT_
						segCmd = (struct macho_segment_command*)cmd;
						uintptr_t start = segCmd->vmaddr;
						uintptr_t end = segCmd->vmaddr + segCmd->vmsize;
						if ( ((start <= loadCommandSegmentVMStart) && (end > loadCommandSegmentVMStart)) //dose not contain the _TEXT_ (左包含)
						   || ((start >= loadCommandSegmentVMStart) && (start < loadCommandSegmentVMEnd)) ) // 右包含
	...
	//可执行文件&引用了其他的动态链接库， 这么这个函数直接返回false
...
ImageLoader* ImageLoaderMachO::instantiateMainExecutable(const macho_header* mh, uintptr_t slide, const char* path, const LinkContext& context)
{
	...
	if ( compressed ) //如果是compress类型的文件
		return ImageLoaderMachOCompressed::instantiateMainExecutable(mh, slide, path, segCount, libCount, context);//解析compressd文件, 初始化并返回ImageLoaderMachOCompressed对象
	else
#if SUPPORT_CLASSIC_MACHO
...
bool ImageLoaderMachO::needsAddedLibSystemDepency(unsigned int libCount, const macho_header* mh)
{
	...
	for (uint32_t i = 0; i < cmd_count; ++i) {
		switch (cmd->cmd) {
			...
			case LC_LOAD_UPWARD_DYLIB:
				return false;
			case LC_ID_DYLIB:	//for dynamic library
				{
				const dylib_command* dylibID = (dylib_command*)cmd;
				const char* installPath = (char*)cmd + dylibID->dylib.name.offset;	//get the path
