...
{
#if CPU_SUBTYPES_SUPPORTED	//CPU SubType==>CPU确切的类型
	...
	if ( mh->magic == sMainExecutableMachHeader->magic ) {//check magic number
		if ( mh->cputype == sMainExecutableMachHeader->cputype ) { //check cpu type(ppc, x86, x64)
			if ( (mh->cputype & CPU_TYPE_MASK) == sHostCPU ) {	//arm, intel .etc
...
// make an ImageLoader* for the already mapped in main executable.
	 ...
	 *  <#Description#>
	 *
	 *  @param mh    the header of mach-o
	 *  @param slide aslr
	 *  @param path  the path of execute file
	 *
...
static ImageLoader* instantiateFromLoadedImage(const macho_header* mh, uintptr_t slide, const char* path)
{
	...
	if ( isCompatibleMachO((const uint8_t*)mh, path) ) {
		ImageLoader* image = ImageLoaderMachO::instantiateMainExecutable(mh, slide, path, gLinkContext);	//返回对应的对象， 
		addImage(image);
		return image;
	}
...
static int __attribute__((noinline)) _shared_region_check_np(uint64_t* start_address)
{
	 ...
	 */
		return syscall(294, start_address);	//#define	SYS_shared_region_check_np 294
	return -1;
}
...
int openSharedCacheFile()
{
	...
	strlcat(path, DYLD_SHARED_CACHE_BASE_NAME ARCH_NAME, MAXPATHLEN);	//get the full path
...
static void mapSharedCache()
{
	...
	if ( _shared_region_check_np(&cacheBaseAddress) == 0 )	//查看是内存是否已经映射好了， 这里有一个系统的调用
	{
		...
		struct dyld_cache_header
		{
			...
			uint32_t	mappingOffset;			// file offset to first dyld_cache_mapping_info	//第一个dyld_cache_mapping_info所在的位置
			uint32_t	mappingCount;			// number of dyld_cache_mapping_info entries	//当前dyld_cache_mapping_info的个数
			uint32_t	imagesOffset;			// file offset to first dyld_cache_image_info	//第一个dyld_cache_image_info所在的位置
			uint32_t	imagesCount;			// number of dyld_cache_image_info entries		//个数
		...
		//读取当前cache的头部
		sSharedCache = (dyld_cache_header*)cacheBaseAddress;			// if we don't understand the currently mapped shared cache, then ignore
		//判断头部
		if ( strcmp(sSharedCache->magic, ARCH_CACHE_MAGIC) != 0 ) {	//首先验证一下头部
			//如果不满足当前的要求
			sSharedCache = NULL;
			if ( gLinkContext.verboseMapping ) {
		...
		//==>验证当前的header至少存在slideInfoSize这个field
		if ( (header->mappingOffset >= 0x48) && (header->slideInfoSize != 0) ) {
			// solve for slide by comparing loaded address to address of first region
			const uint8_t* loadedAddress = (uint8_t*)sSharedCache;//(header的加载起始地址);
			const dyld_cache_mapping_info* const mappings = (dyld_cache_mapping_info*)(loadedAddress+header->mappingOffset);	//记录下当前的maping
			const uint8_t* preferedLoadAddress = (uint8_t*)(long)(mappings[0].address);	//第一个mapping_info所映射的起始地址
			sSharedCacheSlide = loadedAddress/*header's address*/ - preferedLoadAddress;//header的地址与第一个mapping_info所映射的起始地址之间的差值
			dyld::gProcessInfo->sharedCacheSlide = sSharedCacheSlide;	//不知道记录的具体原因
			//dyld::log("sSharedCacheSlide=0x%08lX, loadedAddress=%p, preferedLoadAddress=%p\n", sSharedCacheSlide, loadedAddress, preferedLoadAddress);
		}
		// if cache has a uuid, copy it 
		if ( header->mappingOffset >= 0x68 ) { //has the field of uuid
			memcpy(dyld::gProcessInfo->sharedCacheUUID, header->uuid, 16);
		}
	}
	else {	//如果当前的vm还没有映射好， 那么我们需要映射
		...
		//在我当前的电脑上， dyldshare的路径是/var/db/dyld/dyld_shared_cache_x86_64h
				...
				if ( strcmp(header->magic, ARCH_CACHE_MAGIC) == 0 ) {	//检查magic number
					...
					/*
					 struct dyld_cache_mapping_info {
						uint64_t	address;	//这个应该是当前的cache当前映射的起始地址
						uint64_t	size;
						uint64_t	fileOffset;
						uint32_t	maxProt;
						uint32_t	initProt;
					 ...
					 */
					//shared_file_mapping_np ==> 这个是kernel对应的部分。
					...
					int readWriteMappingIndex = -1;	//有r/w的区域的偏移位置
					int readOnlyMappingIndex = -1;	//ro的区域的偏移位置
					...
					if ( fstat(fd, &stat_buf) == 0 ) {
						...
						for (const dyld_cache_mapping_info* p = fileMappingsStart; p < fileMappingsEnd; ++p, ++i) {
							mappings[i].sfm_address		= p->address;		//当前的加载的地址
							mappings[i].sfm_size		= p->size;			//当前区域的大小
							mappings[i].sfm_file_offset	= p->fileOffset;	//当前段相对于文件的偏移地址
							mappings[i].sfm_max_prot	= p->maxProt;		//最大的保护标志
							mappings[i].sfm_init_prot	= p->initProt;		//初始保护标志
							// rdar://problem/5694507 old update_dyld_shared_cache tool could make a cache file
							// that is not page aligned, but otherwise ok.
							//对齐
							if ( p->fileOffset+p->size > (uint64_t)(stat_buf.st_size+4095 & (-4096)) )	//如果当前的段所在的段超过了文件的地址， 那么应该是cache这个文件出了问题
							{
						...
						uint32_t signatureSize = header->codeSignatureSize;	//当前shared-cache的大小
						// zero size in header means signature runs to end-of-file
						if ( signatureSize == 0 )
							signatureSize = stat_buf.st_size - header->codeSignatureOffset;	//如果是0， 那么， 当前签名的大小是这个文件的剩余部分的总和
						if ( signatureSize != 0 ) {
							//设置codesign这个部分的基本信息（大小， 地址， 最大和初始化保护模式）
					...
					if ( header->imagesCount * sizeof(dyld_cache_image_info) + header->imagesOffset < 8192 ) {
						...
						//dyld_cache_image_info==>这个是cache文件对应的一些引用库。
						if ( my_stat("/usr/lib/libSystem.B.dylib", &stat_buf) == 0 ) {
							const dyld_cache_image_info* images = (dyld_cache_image_info*)&firstPages[header->imagesOffset];	//start
							const dyld_cache_image_info* const imagesEnd = &images[header->imagesCount];						//end
							for (const dyld_cache_image_info* p = images; p < imagesEnd; ++p) {
 								if ( ((time_t)p->modTime == stat_buf.st_mtime) && ((ino_t)p->inode == stat_buf.st_ino) ) {
									foundLibSystem = true;	//找到了/usr/lib/libSystem.B.dylib
									break;
								}
							}					
						}
		...
		uintptr_t* startGlue)
{
	...
	try {
		...
		//instantiat=举例说明
		//初始化对象， 成功(compressed or tranditional)
		sMainExecutable = instantiateFromLoadedImage(mainExecutableMH, mainExecutableSlide, sExecPath);
		gLinkContext.mainExecutable = sMainExecutable;
		gLinkContext.processIsRestricted = sProcessIsRestricted;
		gLinkContext.mainExecutableCodeSigned = hasCodeSignatureLoadCommand(mainExecutableMH);//判断是否有LC_CODESIGN这个字段
		// load shared cache
